	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(component->dev, "es8389_dmic_set: start\n");

	val = ucontrol->value.integer.value[0];
	dev_dbg(component->dev, "es8389_dmic_set: val=%u\n", val);
	
	if (val > 1) {
		dev_err(component->dev, "es8389_dmic_set: invalid val %u\n", val);
		return -EINVAL;
	}

	if (val) {
		dev_dbg(component->dev, "es8389_dmic_set: enabling DMIC\n");
		...
		dev_dbg(component->dev, "es8389_dmic_set: disabling DMIC\n");
	...
	if (changed1 & changed2) {
		ret = snd_soc_dapm_mux_update_power(dapm, kcontrol, val, e, NULL);
		dev_dbg(component->dev, "es8389_dmic_set: mux update power ret=%d\n", ret);
	} else {
		ret = 0;
	}

	dev_dbg(component->dev, "es8389_dmic_set: end, ret=%d\n", ret);
	return ret;
}
...
static inline int get_coeff(int mclk, int rate)
{
	...
	pr_debug("get_coeff: start, mclk=%d, rate=%d\n", mclk, rate);

	for (i = 0; i < ARRAY_SIZE(coeff_div); i++) {
		if (coeff_div[i].rate == rate &&  coeff_div[i].mclk == mclk) {
			pr_debug("get_coeff: found match at index %d\n", i);
			pr_debug("get_coeff: end, ret=%d\n", i);
			return i;
		}
	}
	
	pr_debug("get_coeff: no match found\n");
	pr_debug("get_coeff: end, ret=%d\n", -EINVAL);
	return -EINVAL;
}
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	dev_dbg(component->dev, "es8389_set_dai_sysclk: start, clk_id=%d, freq=%u, dir=%d\n", 
		clk_id, freq, dir);

	es8389->sysclk = freq;
	dev_dbg(component->dev, "es8389_set_dai_sysclk: set sysclk to %u\n", freq);

	dev_dbg(component->dev, "es8389_set_dai_sysclk: end, ret=0\n");
	return 0;
}
...
static int es8389_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	dev_dbg(component->dev, "es8389_set_dai_fmt: start, fmt=0x%x\n", fmt);

	switch(fmt & SND_SOC_DAIFMT_MASTER_MASK)
	{
		case SND_SOC_DAIFMT_CBM_CFM:
			dev_dbg(component->dev, "es8389_set_dai_fmt: setting master mode\n");
			regmap_update_bits(es8389->regmap, ES8389_MASTER_MODE,
				ES8389_MASTER_MODE_EN, ES8389_MASTER_MODE_EN);
			es8389->mastermode = 1;
			break;
		case SND_SOC_DAIFMT_CBS_CFS:
			dev_dbg(component->dev, "es8389_set_dai_fmt: setting slave mode\n");
			es8389->mastermode = 0;
			break;
		default:
			dev_err(component->dev, "es8389_set_dai_fmt: invalid master mask\n");
			return -EINVAL;
	}
	...
	dev_dbg(component->dev, "es8389_set_dai_fmt: updating format registers with state=0x%x\n", state);
	...
	dev_dbg(component->dev, "es8389_set_dai_fmt: end, ret=0\n");
	return 0;
}
			...
			struct snd_soc_dai *dai)
{
	...
	dev_dbg(component->dev, "es8389_pcm_hw_params: start\n");
	dev_dbg(component->dev, "es8389_pcm_hw_params: rate=%d, channels=%d, width=%d\n",
		params_rate(params), params_channels(params), params_width(params));

	switch (params_format(params)) {
	case SNDRV_PCM_FORMAT_S16_LE:
		dev_dbg(component->dev, "es8389_pcm_hw_params: setting S16_LE format\n");
		state |= ES8389_S16_LE;
		break;
	case SNDRV_PCM_FORMAT_S20_3LE:
		dev_dbg(component->dev, "es8389_pcm_hw_params: setting S20_3LE format\n");
		state |= ES8389_S20_3_LE;
		break;
	case SNDRV_PCM_FORMAT_S18_3LE:
		dev_dbg(component->dev, "es8389_pcm_hw_params: setting S18_3LE format\n");
		state |= ES8389_S18_LE;
		break;
	case SNDRV_PCM_FORMAT_S24_LE:
		dev_dbg(component->dev, "es8389_pcm_hw_params: setting S24_LE format\n");
		state |= ES8389_S24_LE;
		break;
	case SNDRV_PCM_FORMAT_S32_LE:
		dev_dbg(component->dev, "es8389_pcm_hw_params: setting S32_LE format\n");
		state |= ES8389_S32_LE;
		break;
	default:
		dev_err(component->dev, "es8389_pcm_hw_params: unsupported format\n");
		return -EINVAL;
	}

	dev_dbg(component->dev, "es8389_pcm_hw_params: updating format registers with state=0x%x\n", state);
	...
	if (es8389->mclk_src == ES8389_SCLK_PIN) {
		dev_dbg(component->dev, "es8389_pcm_hw_params: using SCLK_PIN as mclk source\n");
		...
		dev_dbg(component->dev, "es8389_pcm_hw_params: calculated sysclk=%u\n", es8389->sysclk);
	}

	dev_dbg(component->dev, "es8389_pcm_hw_params: getting clock coefficients\n");
	coeff = get_coeff(es8389->sysclk, params_rate(params));
	if (coeff >= 0) {
		dev_dbg(component->dev, "es8389_pcm_hw_params: found coeff at index %d, applying clock settings\n", coeff);
		...
		dev_dbg(component->dev, "es8389_pcm_hw_params: clock settings applied successfully\n");
	} else {
		dev_warn(component->dev, "es8389_pcm_hw_params: Clock coefficients do not match");
	}

	dev_dbg(component->dev, "es8389_pcm_hw_params: end, ret=0\n");
	return 0;
}
			...
			enum snd_soc_bias_level level)
{
	...
	dev_dbg(component->dev, "es8389_set_bias_level: start, level=%d\n", level);

	switch (level) {
	case SND_SOC_BIAS_ON:
		dev_dbg(component->dev, "es8389_set_bias_level: setting BIAS_ON\n");
		ret = clk_prepare_enable(es8389->mclk);
		if (ret) {
			dev_err(component->dev, "es8389_set_bias_level: failed to enable mclk, ret=%d\n", ret);
			return ret;
		}

		dev_dbg(component->dev, "es8389_set_bias_level: configuring registers for ON state\n");
		...
		dev_dbg(component->dev, "es8389_set_bias_level: BIAS_ON configuration complete\n");
		break;
	case SND_SOC_BIAS_PREPARE:
		dev_dbg(component->dev, "es8389_set_bias_level: setting BIAS_PREPARE\n");
		break;
	case SND_SOC_BIAS_STANDBY:
		dev_dbg(component->dev, "es8389_set_bias_level: setting BIAS_STANDBY\n");
		...
		dev_dbg(component->dev, "es8389_set_bias_level: disabling mclk\n");
		clk_disable_unprepare(es8389->mclk);
		dev_dbg(component->dev, "es8389_set_bias_level: BIAS_STANDBY configuration complete\n");
		break;
	case SND_SOC_BIAS_OFF:
		dev_dbg(component->dev, "es8389_set_bias_level: setting BIAS_OFF\n");
		break;
	}
	
	dev_dbg(component->dev, "es8389_set_bias_level: end, ret=0\n");
	return 0;
}
...
static int es8389_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	...
	dev_dbg(component->dev, "es8389_mute: start, mute=%d, direction=%d\n", mute, direction);

	if (mute) {
		if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
			dev_dbg(component->dev, "es8389_mute: muting playback\n");
			regmap_update_bits(es8389->regmap, ES8389_DAC_FORMAT_MUTE,
						0x03, 0x03);
		} else {
			dev_dbg(component->dev, "es8389_mute: muting capture\n");
			regmap_update_bits(es8389->regmap, ES8389_ADC_FORMAT_MUTE,
						0x03, 0x03);
		}
	} else {
		if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
			dev_dbg(component->dev, "es8389_mute: unmuting playback\n");
			regmap_update_bits(es8389->regmap, ES8389_DAC_FORMAT_MUTE,
						0x03, 0x00);
		} else {
			dev_dbg(component->dev, "es8389_mute: unmuting capture\n");
			regmap_update_bits(es8389->regmap, ES8389_ADC_FORMAT_MUTE,
						0x03, 0x00);
		}
	}

	dev_dbg(component->dev, "es8389_mute: end, ret=0\n");
	return 0;
}
...
static void es8389_init(struct snd_soc_component *component)
{
	...
	dev_dbg(component->dev, "es8389_init: start\n");

	dev_dbg(component->dev, "es8389_init: configuring basic registers\n");
	...
	dev_dbg(component->dev, "es8389_init: configuring analog control\n");
	...
	dev_dbg(component->dev, "es8389_init: configuring PGA and mode switches\n");
	...
	dev_dbg(component->dev, "es8389_init: configuring microphone settings\n");
	...
	dev_dbg(component->dev, "es8389_init: configuring clock settings\n");
	...
	dev_dbg(component->dev, "es8389_init: setting initial mute state\n");
	...
	dev_dbg(component->dev, "es8389_init: end\n");
}
...
static int es8389_suspend(struct snd_soc_component *component)
{
	...
	dev_dbg(component->dev, "es8389_suspend: start\n");

	dev_dbg(component->dev, "es8389_suspend: setting bias level to STANDBY\n");
	es8389_set_bias_level(component, SND_SOC_BIAS_STANDBY);
	
	dev_dbg(component->dev, "es8389_suspend: enabling cache only mode\n");
	regcache_cache_only(es8389->regmap, true);
	regcache_mark_dirty(es8389->regmap);

	dev_dbg(component->dev, "es8389_suspend: end, ret=0\n");
	return 0;
}
...
static int es8389_resume(struct snd_soc_component *component)
{
	...
	dev_dbg(component->dev, "es8389_resume: start\n");

	dev_dbg(component->dev, "es8389_resume: disabling cache only mode\n");
	regcache_cache_only(es8389->regmap, false);
	regcache_cache_bypass(es8389->regmap, true);
	
	dev_dbg(component->dev, "es8389_resume: reading reset register\n");
	regmap_read(es8389->regmap, ES8389_RESET, &regv);
	regcache_cache_bypass(es8389->regmap, false);

	dev_dbg(component->dev, "es8389_resume: reset register value=0x%x\n", regv);
	if (regv == 0xff) {
		dev_dbg(component->dev, "es8389_resume: chip needs reinitialization\n");
		es8389_init(component);
	} else {
		dev_dbg(component->dev, "es8389_resume: setting bias level to ON\n");
		es8389_set_bias_level(component, SND_SOC_BIAS_ON);
	}

	dev_dbg(component->dev, "es8389_resume: syncing register cache\n");
	regcache_sync(es8389->regmap);

	dev_dbg(component->dev, "es8389_resume: end, ret=0\n");
	return 0;
}
