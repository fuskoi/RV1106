...
static struct proc_ops *rtw_file_ops_to_proc_ops(const struct file_operations *fops)
{
	static struct proc_ops proc_ops;
	_rtw_memset(&proc_ops, 0, sizeof(proc_ops));
	if (fops->open)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
	{
		struct proc_ops *proc_ops = rtw_file_ops_to_proc_ops(fops);
