...
{
	dev_info(component->dev, "es8323_reset start\n");
	snd_soc_component_write(component, ES8323_CONTROL1, 0x80);
	dev_info(component->dev, "es8323_reset end\n");
				 ...
				 int clk_id, unsigned int freq, int dir)
{
	...
	dev_info(component->dev, "es8323_set_dai_sysclk start\n");
	...
	// 	dev_info(component->dev, "es8323_set_dai_sysclk end\n");
	// 	return 0;
	// }
...
static int es8323_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	dev_info(component->dev, "es8323_set_dai_fmt start\n");
	...
	case SND_SOC_DAIFMT_CBS_CFS:	/* SLAVE MODE */
		...
		break;
	default:
		dev_info(component->dev, "es8323_set_dai_fmt end\n");
		return -EINVAL;
	}
...
{
	dev_info(dai->component->dev, "es8323_pcm_startup start\n");
	dev_info(dai->component->dev, "es8323_pcm_startup end\n");
	return 0;
}
				...
				struct snd_soc_dai *dai)
{
	...
	dev_info(component->dev, "es8323_pcm_hw_params start\n");
	...
	if (i == NR_SUPPORTED_MCLK_LRCK_RATIOS) {
		...
		dev_info(component->dev, "es8323_pcm_hw_params end\n");
		return -EINVAL;
	}
...
{
	dev_info(dai->component->dev, "es8323_mute start\n");
	dev_info(dai->component->dev, "es8323_mute end\n");
	return 0;
}
				 ...
				 enum snd_soc_bias_level level)
{
	...
	dev_info(component->dev, "es8323_set_bias_level start\n");
	...
	case SND_SOC_BIAS_PREPARE:
		...
		if (snd_soc_component_get_bias_level(component) == SND_SOC_BIAS_ON) {
			...
			if (ret) {
				dev_info(component->dev, "es8323_set_bias_level end\n");
				return ret;
			}
		}
...
static int es8323_suspend(struct snd_soc_component *component)
{
	...
	dev_info(component->dev, "es8323_suspend start\n");
	...
	dev_info(component->dev, "es8323_suspend end\n");
	return 0;
}
...
static int es8323_resume(struct snd_soc_component *component)
{
	...
	dev_info(component->dev, "es8323_resume start\n");
	...
	dev_info(component->dev, "es8323_resume end\n");
	return 0;
}
...
{
	dev_info(component->dev, "es8323_remove start\n");
	es8323_set_bias_level(component, SND_SOC_BIAS_OFF);
	dev_info(component->dev, "es8323_remove end\n");
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		...
		dev_info(&i2c->dev, "es8323_i2c_probe end_3\n");
		return ret;
	}
...
{
	dev_info(&client->dev, "es8323_i2c_remove start\n");
	snd_soc_unregister_component(&client->dev);
	dev_info(&client->dev, "es8323_i2c_remove end\n");
	return 0;
}
...
static void es8323_i2c_shutdown(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "es8323_i2c_shutdown start\n");
	...
	dev_info(&client->dev, "es8323_i2c_shutdown end\n");
}
...
static int __init es8323_init(void)
{
	...
	if (ret != 0)
		printk("Failed to register es8323 i2c driver\n");
	return ret;
}
